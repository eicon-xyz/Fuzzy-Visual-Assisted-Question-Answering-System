...
- **S2.5（gui-agents v0.2.5，2025-08-01，官方 README 口径、无独立论文）**：`s2_5/` 源码里 **UIAgent 只剩 Worker+ACI、Manager 类被删**——Simular 自己在 v0.2.5 就实践了 S3 的「去层级 flat」结论（四段式提示词与 hotkey 优先规则保留）。⇒ 对 HAJIMI：**层级规划不是必需品，ACI+四段式+轨迹反思才是**。
...
- 更优形态 = S2 式**多专家路由**而非单换模型：①结构专家=UIA（已有）②**OCR 词级 span 专家**（选文本/词编辑，S2 去之 -5.4pt 子任务成功率，实现=RapidOCR+现成 bbox，成本最低）③视觉专家只管自绘/canvas/图片区（**选 GUI-Owl（SSP 54.9）或 UI-TARS-1.5-7B（49.6），勿选 OS-Atlas——SSP 仅 18.9 高分辨率专业软件短板+仓库停滞**）④Office 结构写入专家（COM/ValuePattern，去之 -4.3pt）。接入路径已核实：**SiliconFlow 未上架 GUI-Owl/UI-TARS（2026-09 官方模型清单全文检索零命中，其视觉线可取用的是 Qwen3-VL 系但 GUI grounding 成绩未验证）**；第一方服务化 = **DashScope**（OSWorld 官方 `mm_agents/owl_agent.py` 原生支持 `engine="dash"`），或 **自托管 vLLM/SGLang**（HF `mPLUG/GUI-Owl-1.5-2B/8B/32B-Instruct`，Apache-2.0）；均走 OpenAI 兼容协议，HAJIMI `llm/providers.py` 直接复用。最省事的验证实验：vLLM 起 GUI-Owl-1.5-8B，用 Agent-S2 `grounding.py::generate_coords` 的极简 prompt（`"Query:{指称表达式}\nOutput only the coordinate..."`）先做离线 grounding A/B，再谈入链路；坐标反算注意口径（1000×1000 归一 vs Qwen2.5-VL 绝对像素需 smart_resize）。
...
| 2.4 | **回归评测台（建议提前到 P1 并行做）**：移植 WAA 任务 schema（初始断言+成功判据 env-check oracle），30-50 个真实失败任务+VM 快照沙箱；口径=**≥3 参数种子 + All-Pass@4**（单 seed 会骗人：AndroidWorld 同集 27.6/26.3/33.2；单次÷稳定性=生产折扣）；失败按 planning/grounding-coverage/execution/recovery 四类打标（对齐文献口径）；snapshot 加延迟遥测（WAA：UIA 查询可达秒~分钟级） | cua harness（同任务跨环境 90%→9%）、RoTS、AndroidWorld seed 方差 | **✅ 已提前实施（P0.5-T1~T4：0e16b9a9/8698cdec/3fe06a64 + 手册）**：`server_A/eval/` 遥测（token/轮次/gates/loops/expect/快照延迟→runs.jsonl+DB 回写+engine 崩溃保护壳）+ 任务集 schema/20 任务（每条 P0 ≥2 暴露任务；oracle 全机器判据，自评/视觉类显式禁用）+ runner/report（seeds×repeats、All-Pass@4 主口径、四类失败打标、A/B delta）。**欠账（诚实记账）**：任务全部 `calibrated:false`，需 Windows 双向校准后才算真 KPI（流程见 `server_A/eval/HOWTO_WINDOWS.md`，含基线 worktree 回测）；用户真实失败任务待补；VM 快照未上（幂等 cleanup 替代，环境抖动风险已知） |
