...
class TestBuildToolDefinitions:
    def test_total_count_is_25(self):
        ...
        assert len(tools) == 25  # P0.5-B4 四件套 +4
    ...
    def test_desktop_tools_count_is_17(self):
        desktop = [
            t["function"]["name"]
            for t in _build_tool_definitions()
            if not t["function"]["name"].startswith("browser_")
        ]
        assert len(desktop) == 17  # P0.5-B4: right_click/set_range/select_menu_path/window_action
        ...
        assert "report_infeasible" in desktop  # 0.7 显式不可行终止
        assert "ask_user" in desktop  # 0.7 求助终止
