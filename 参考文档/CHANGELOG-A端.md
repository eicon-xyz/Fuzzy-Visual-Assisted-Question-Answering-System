...
- [ ] SeeClick / YOLO 评估（✅ 2026-07-02 评估完成，报告见 `项目文档/SeeClick-YOLO-评估报告.md`；结论：暂不入主链路，优先 GroundingDINO 级联）
- [x] `ProcessResponse` 可选返回 `reference_resolution` 写入 OpenAPI example（✅ `HealthResponse` 已扩展 `detector_backend` + `omniparser_ready`）
- [x] LLM 管线升级：迁移至 Qwen3.6 多模态（✅ 2026-07-02）
...
| 2026-07-02 | **LLM 管线升级**：DeepSeek → Qwen3.6 多模态（SiliconCloud），3 条 LLM 路径统一；`call_deepseek()` 支持 `image_data`；`client.py` 增加 `_get_api_config()` 优先级链路（`LLM_*` > `DEEPSEEK_*`）；`replanner.py` / `router.py` 切到统一 LLM 客户端；SoM 标注图传入 LLM；Prompt 增加 WPS 菜单栏 few-shot；`.env.example` / `README.md` 同步 | `llm/client.py`, `llm/prompt.py`, `planning/router.py`, `planning/replanner.py`, `config.py`, `.env`, `.env.example`, `README.md` |
| 2026-07-02 | **代码核查 + 文档同步**：确认 `/relocate` 端点已实现（`demo.py:238-300`）；`router.py` 第20行附近无误时 TODO；`README.md` 第145行标注 `llm_ai.py` 为 DEPRECATED；`A_实现优先级.md` 修正 §15.1 矛盾描述；更新 CHANGELOG 待办项状态 | `README.md`, `A_实现优先级.md`, `CHANGELOG-A端.md` |
| 2026-07-02 | **接口契约同步 + 检测器评估**：`api-contract-demo.yaml` 补充 `reference_resolution`、`detection_meta`、`redline`、RedlineInfo 及 `/inspect`、`/relocate` 端点；完成 SeeClick / YOLO 评估报告，结论为暂不入主链路、优先 GroundingDINO 级联 | `api-contract-demo.yaml`, `项目文档/SeeClick-YOLO-评估报告.md`, `CHANGELOG-A端.md`, `A_实现优先级.md` |
