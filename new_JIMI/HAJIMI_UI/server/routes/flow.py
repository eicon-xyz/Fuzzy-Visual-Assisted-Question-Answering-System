...
async def metrics(api_path: str = "/api/demo/process", key: str = Depends(_auth)):
    random.seed(42)
    return {"api_path": api_path, "granularity": "5m",
            "data": [{"time": f"{h:02d}:{m:02d}", "qps": 20 + random.randint(0, 40),
...
async def versions(key: str = Depends(_auth)):
    return {"versions": [{"version": "v2.1.0", "count": 34, "pct": 80.9},
                         {"version": "v2.0.5", "count": 6, "pct": 14.3},
                         {"version": "v1.9.0", "count": 2, "pct": 4.8}], "total_clients": 42}
