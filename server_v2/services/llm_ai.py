...
from server_v2.models.schemas import UIElement, ProcessResponse, Intent


def classify_intent(query: str) -> Tuple[str, str, float]:
    """意图分类入口"""
    from server_v2.services.intent import classify_intent as new_classify_intent
...
def generate_steps(query: str, elements: Optional[List[UIElement]] = None) -> List[dict]:
    """步骤生成入口"""
    from server_v2.services.planning import generate_steps as new_generate_steps
...
def process_query(query: str, image_base64: Optional[str] = None) -> ProcessResponse:
    """核心流程入口"""
    from server_v2.services.planning import process_query as new_process_query
    return new_process_query(query, image_base64)


def call_deepseek(query: str, timeout: int = 30):
    """DeepSeek 调用入口"""
    from server_v2.services.llm import call_deepseek as new_call_deepseek
