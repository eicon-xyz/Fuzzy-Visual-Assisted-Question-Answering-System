...
from server_v2.services.llm import call_deepseek
from server_v2.services.omniparser_client import parse_screenshot, parse_screenshot_full
from server_v2.services.planning.annotation import build_annotation
from server_v2.services.redline_service import check_redline
from server_v2.services.planning.complexity_router import score_complexity, generate_l2_steps
...
def process_query(query: str, image_base64: Optional[str] = None) -> ProcessResponse:
    ...
    from server_v2.services.llm_ai import classify_intent, detect_reference_type
    ...
    # 能力路由（新增 — 不替换现有 L2/L3，作为补充维度）
    from server_v2.services.planning.complexity_router import route_capability
    capability = route_capability(query)
    ...
    if image_base64:
        ...
        if parse_result.elements:
            ...
            if detection_meta:
                ...
                detection_meta['capability'] = capability.value
        else:
            scenario = _choose_scenario(query)
    ...
    from server_v2.services.planning.risk_scorer import score_step
    from server_v2.services.planning.coordinate_validator import validate_coordinate
    ...
    for i, raw in enumerate(raw_steps):
        ...
        if element:
            ...
            # 坐标后处理校验
            if element.center:
                coord_result = validate_coordinate(
                    element.center, elements,
                    label=element.text or "",
                )
                annotation.confidence = coord_result.get("confidence", 0.8)
        else:
            annotation = None

        # 风险评分
        step_risk = score_step(raw.get("action", ""), raw.get("description", ""))
...
def relocate_step(
    ...
    if matched_element:
        ...
        # 坐标后处理校验
        if matched_element.center:
            from server_v2.services.planning.coordinate_validator import validate_coordinate as vc
            coord_result = vc(matched_element.center, elements, label=matched_element.text or "")
            annotation.confidence = coord_result.get("confidence", 0.8)
