# HAJIMI B 端接口总结（对 A / 对 C）

> **维护人**：B 端（前端 / 桌面应用）  
> **版本**：1.1.0  
> **依据**：[`设计文档V2.md`](../设计文档V2.md) 第九章分工、[`b-c-api-contract.md`](../b-c-api-contract.md)、[`api-contract-demo.yaml`](../api-contract-demo.yaml)、[`CHANGELOG-B端.md`](CHANGELOG-B端.md)、[`server/docs/CHANGELOG-A端.md`](../server/docs/CHANGELOG-A端.md)  
> **最后更新**：2026-07-01
...
| 配置项 | 环境变量 | 默认值 | 说明 |
|--------|----------|--------|------|
| A 端地址 | `HAJIMI_API_URL` | `http://127.0.0.1:8010` | 与 A 端 uvicorn 端口一致 |
| Demo Key | `HAJIMI_DEMO_KEY` | `hajimi-demo-2026` | 请求头 `X-Demo-Key` |
| 纯 Mock | `HAJIMI_MOCK_ONLY=1` | 关 | 不走 A，检验模式不可用 |
| Mock 降级 | `HAJIMI_MOCK_FALLBACK=1` | 关 | A 不可达时 process 回退本地 Mock |

**B 端实现入口**：`core/api_client.py`
...
> **契约文件**：[`api-contract-demo.yaml`](../api-contract-demo.yaml)（`/inspect` 已同步；`/relocate` 与扩展 health 字段**尚未写入 YAML**，以本文与 A 端代码为准）。

### 3.3 A 端改动汇入（B 联调必读）

以下汇总自 [`server/docs/CHANGELOG-A端.md`](../server/docs/CHANGELOG-A端.md)，是 B 端对接 A 时必须知晓的**行为变更**。
...
| 字段 | B 端用途 |
|------|----------|
| `detector_backend` | 缺失 → 判定旧版/多开 A 端，提示 `stop_all.bat` |
| `omniparser_ready` | `false` → 阻止 inspect/process，提示先启 OmniParser |
...
| **学校 A800 GPU 容器** | 容器内 GPU 版 OmniParser | 见 [`校园GPU与OmniParser环境速查.md`](校园GPU与OmniParser环境速查.md) §三；本地 Windows 默认 CPU 推理 |
...
| 设置/服务 | `ui/main_widget.py`, `core/service_manager.py` | health 预检、启停 A/OmniParser |
...
| Demo API 契约（YAML） | [`api-contract-demo.yaml`](../api-contract-demo.yaml) |
| B–C 详细契约 | [`b-c-api-contract.md`](../b-c-api-contract.md) |
| A 端改动全记录 | [`server/docs/CHANGELOG-A端.md`](../server/docs/CHANGELOG-A端.md) |
| B 端改动全记录 | [`CHANGELOG-B端.md`](CHANGELOG-B端.md) |
| DAY2 工作计划 | [`DAY2-工作内容.md`](DAY2-工作内容.md) |
| 校园 GPU / OmniParser 速查 | [`校园GPU与OmniParser环境速查.md`](校园GPU与OmniParser环境速查.md) |
