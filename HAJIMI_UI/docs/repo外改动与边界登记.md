# repo 外改动与边界登记（L5 外接 new_JIMI）

> **读者**：A / B / C 三端；合并 PR 或联调前必读  
> **原则**：B 端改造集中在 [`HAJIMI_UI/`](../)；**不修改** `new_JIMI` 业务代码；repo 外变更在本文件登记。  
...
| 路径 | 类型 | 维护者 | 说明 |
|------|------|--------|------|
| [`new_JIMI/HAJIMI_UI/server/.env.example`](../../new_JIMI/HAJIMI_UI/server/.env.example) | **B 新增模板** | A 复制使用 | L5 Sidecar `:8011` 的 LLM/Omni/Demo-Key 占位；**非**业务代码 |

### 明确未改（A 零改业务代码）

| 路径 | 说明 |
|------|------|
| `new_JIMI/HAJIMI_UI/server/**/*.py` | executor / agent / routes — **无 B 端 diff** |
| `new_JIMI/HAJIMI_UI/scripts/` | A 侧启动脚本 — B 不覆盖 |
...
## 2. new_JIMI 目录角色（A 主责）

```
repo 根/
├── HAJIMI_UI/              ← B 主仓库：8010 canonical + B UI + 启动 Sidecar 的 bat
└── new_JIMI/HAJIMI_UI/     ← A L5 源码：8011 Sidecar（独立 venv + .env）
    └── server/
        ├── .env.example    ← B 提供的模板（§1 唯一 repo 外文件改动）
        ├── .env            ← A 本地创建，**不入 git**
...
| **更新** | A `git pull` → restart 8011；B **不 merge** executor 进 canonical |
| **B 启动** | [`HAJIMI_UI/scripts/start_l5_sidecar.bat`](../scripts/start_l5_sidecar.bat) 薄封装 call `new_JIMI/.../start_server.bat` |
| **路径覆盖** | 环境变量 `HAJIMI_L5_ROOT` 指向 Sidecar 根目录 |
...
| `env_sync` | **仅写** `HAJIMI_UI/server/.env`，**不写** new_JIMI |
...
| 路径 | 问题 | 应以何为准 |
|------|------|------------|
| [`工作进度/测试指南.md`](../../工作进度/测试指南.md) | 写 A 在 new_JIMI `:8010` | 8010 = `HAJIMI_UI/server`；8011 = new_JIMI L5 |
...
# 编辑 server\.env：LLM_API_KEY、OMNIPARSER_URL（与 8010 一致）
set HAJIMI_PORT=8011
scripts\start_server.bat
curl http://127.0.0.1:8011/api/demo/health/live
```

或由 B 一键：`HAJIMI_UI\scripts\start_l5_sidecar.bat`
...
- **何时更新**：new_JIMI 侧新增 B 可见文件；或 repo 外文档与架构再次分叉时
