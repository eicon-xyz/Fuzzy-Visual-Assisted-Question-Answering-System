...
## Demo 实现状态（2026-07，以代码为准）

| 端点 / 能力 | 本文目标 | 当前 [`server/routes/auth.py`](../server/routes/auth.py) | 客户端 |
|-------------|----------|----------------------------------------------------------|--------|
| `POST /login` | 完整 JWT pair + user | **stub**：`{ access_token, expires_in }`，SHA256 | web-admin + B 端弹窗 **已接** |
| `POST /register` | 有 | **未实现** | — |
| `POST /refresh` | 有 | **未实现** | 前端 stub 预留 |
| `POST /logout` | 有 | **未实现** | 仅清本地 session |
| `/api/demo/*` | 可选 Bearer | **X-Demo-Key** | B 端不变 |
| `/api/admin/*` | Bearer | **X-Admin-Key** | web-admin 仍带 Demo Key |

- **演示账号**：`admin` / `demo123`（stub 首次登录可自动建用户）
- B 端本地回退：A 不可达且使用 `admin`/`demo123` 时写入本地演示 session（`local-demo.*` token）
- B 端跳过登录：`set HAJIMI_SKIP_LOGIN=1`
- B 端 session：`%LOCALAPPDATA%/HAJIMI/auth_session.json`
- web-admin session：`localStorage` 键 `hajimi_access_token` 等（见 `web-admin/src/auth/`）
