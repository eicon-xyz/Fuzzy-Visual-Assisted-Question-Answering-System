...
| 6 | **接口文档** | B 对 A/C 接口总结 + A 改动汇入 | `docs/B端接口总结-对A与对C.md` | ✅ 本文档批次 |
...
| 4 | 启动脚本与 README | `scripts/start_server.bat`, `server/README.md` | ✅ |
...
| 交付物 | 路径 | 说明 |
|--------|------|------|
| B 端接口总结（对 A / 对 C） | `docs/B端接口总结-对A与对C.md` | 含 A 成员改动汇总 |
| Demo API 契约 | `api-contract-demo.yaml` | A+B 对齐基准 |
| B–C 接口契约 | `b-c-api-contract.md` | C 联调基准 |
| A 端改动记录 | `server/docs/CHANGELOG-A端.md` | A 维护 |
| B 端改动记录 | `docs/CHANGELOG-B端.md` | B 维护 |
...
| 步骤 | 文档 |
|------|------|
| 连接 Jupyter / VSCode / SSH | [`校园GPU与OmniParser环境速查.md`](校园GPU与OmniParser环境速查.md) §一 |
| 验证 `nvidia-smi` / 显存占用 | 同上 §二 |
| 安装 OmniParser v2（严格版本 + 4 处源码修改） | 同上 §三；细节见 [`OmniParser GPU 环境交接文档.md`](../OmniParser%20GPU%20环境交接文档.md) |
| 与 HAJIMI A 端对接 | [`B端接口总结-对A与对C.md`](B端接口总结-对A与对C.md) §3.3.7 |
...
- B 接口总结：[`B端接口总结-对A与对C.md`](B端接口总结-对A与对C.md)
