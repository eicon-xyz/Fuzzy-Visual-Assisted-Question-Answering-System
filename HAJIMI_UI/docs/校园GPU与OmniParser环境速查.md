...
| 场景 | 说明 |
|------|------|
| **本地 Windows 开发** | RTX 50 系 / 驱动限制，默认 **CPU OmniParser**（~2–4 分钟/帧），见 [`server/docs/CHANGELOG-A端.md`](../server/docs/CHANGELOG-A端.md) |
| **学校 A800 容器** | 按本文 §三 + 交接文档部署 **GPU 版 OmniParser**，可将 `DETECTOR_BACKEND=local_omniparser` 的 URL 指向容器内服务 |
| **B 端** | 仅通过 HTTP 调用 A 端 `/process`、`/inspect`；不关心检测跑在本地 CPU 还是远程 GPU |

联调相关文档：[`B端接口总结-对A与对C.md`](B端接口总结-对A与对C.md)、[`DAY2-工作内容.md`](DAY2-工作内容.md)
