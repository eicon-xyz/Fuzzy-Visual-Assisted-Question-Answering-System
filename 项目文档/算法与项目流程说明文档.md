...
其中 $\alpha=0.3, \beta=8, \gamma=10$（默认值，可被服务端热部署覆盖）。`score < 30` 走 L2，$\geq 30$ 走 L3。该接口预留 `ComplexityRouter` 抽象，便于后续替换为 BERT 分类器。
...
4. else:
5.     img_b64 = render_som_image(som_map)
6.     steps = LLM_multimodal_plan(img_b64, Q, intent_ctx)
7. blueprint = Blueprint(steps, fp_expected=predict_fps(steps))
8. memory.save_blueprint(blueprint)
9. return blueprint
```
...
HAJIMI 的算法体系通过**审计上报 + 配置热部署**实现持续监控与调优：
...
│ 1. 执行任务 L3     │              │                    │
│ 2. 蓝图成功完成    │              │                    │
│ 3. 异步上报        │  ──POST──►  │ 4. 写入审计日志    │
│                    │              │ 5. 失败归因分析    │
│                    │              │ 6. 管理员调优      │
│ 7. 定期拉取配置    │  ◄──GET──   │ 8. 热部署下发      │
└────────────────────┘              └────────────────────┘
```
...
2. **系统亮点**：二级速度路由 + 状态机 + 指纹比对，实现复杂任务的稳定执行；
...
### 一个闭环
**"用户提问 → 屏幕感知 → 意图理解 → 蓝图规划 → 多模态执行 → 指纹验证 → 审计上报"** 的端到端链路，形成完整的操作指引闭环。
