...
> **覆盖范围**：审计上报、配置管理、管理控制台全部 15 个数据接口、模板管理、健康检查
...
| **模板管理** |
| 19 | Admin | GET | `/api/admin/templates/list` | Web 管理面板 | 模板列表（含筛选） |
| 20 | Admin | GET | `/api/admin/templates/detail/{id}` | Web 管理面板 | 模板详情（含步骤预览） |
| 21 | Admin | POST | `/api/admin/templates/review` | Web 管理面板 | 审核/发布/下架模板 |
| **系统** |
...
| `batch[].result` | string | ✅ | `success` / `fail` / `cancel` / `rejected` |
...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `status` | string | `all` | `all` / `published` / `pending_review` / `archived` |
| `category` | string | ❌ | 意图域筛选 |
| `keyword` | string | ❌ | 模板名/描述模糊搜索 |
| `page` | int | 1 | 页码 |
...
#### `GET /api/admin/templates/detail/{template_id}`
...
```json
{
  ...
  "name": "安装微信",
  "intent_category": "operation_guide",
  "description": "引导用户在 Microsoft Store 中搜索并安装微信",
  "status": "published",
  "current_version": "v1.2.0",
  "steps": [
    {"step_index": 1, "action": "打开商店", "description": "点击任务栏商店图标打开 Microsoft Store。"},
    {"step_index": 2, "action": "搜索应用", "description": "在搜索框中输入「微信」并回车。"},
    {"step_index": 3, "action": "点击安装", "description": "点击「获取/安装」按钮。"}
  ],
  "versions": [
    {"version": "v1.2.0", "changed_by": "admin", "changed_at": "2026-06-28T15:30:00Z", "changelog": "优化步骤描述"},
    {"version": "v1.1.0", "changed_by": "system", "changed_at": "2026-06-22T09:00:00Z", "changelog": "自动沉淀：基于 50 次成功执行微调"}
  ],
  "stats": {
    "use_count": 245,
    "success_rate": 0.92,
...
#### `POST /api/admin/templates/review`
...
|----------|------|
| `publish` | 发布到全局模板库 |
| `unpublish` | 下架（用户不可见但仍保留） |
| `reject` | 退回修改（需填写 comment） |
| `archive` | 归档（永久移除） |
...
```json
{
  ...
  "reviewed_at": "2026-06-29T14:40:00Z"
}
```
...
| `/api/templates/*` | `Authorization: Bearer <JWT>` | 同上 |
